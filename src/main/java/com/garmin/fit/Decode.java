...
public class Decode {
   ...
   public boolean read(final InputStream in, final MesgListener mesgListener, final MesgDefinitionListener mesgDefListener) {
      ...
      try {
         while ( (in.available() > 0) && (status == true) ) { // try to read a file while more data is available.
         ...
         case FILE_HDR:
            switch (fileHdrOffset++) {
               ...
               case 7:
                  ...
                  // Check file size
                  if ( (fileDataSize == 0) && (invalidDataSize == false) ) {
                     invalidDataSize = true;
                     headerException = "FIT decode error: File Size is 0.";
                  }
                  break;
               case 8:
                  if (data != '.')
